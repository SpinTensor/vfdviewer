...
void comm_matrix_update_time_max(vgtk_comm_matrix_t *comm_matrix) {
   ...
   while (vfdtrace != NULL) {
      ...
      // and search for the maximum time for the spcific
         ...
         if (message.dtstart_sec > tmax) {
            ...
            if ((show_send && message.dir == send) ||
               ...
               comm_matrix->data[idx] =
                  comm_matrix->data[idx] > message.time_sec ?
                     comm_matrix->data[idx] :
                     message.time_sec;
...
void comm_matrix_update_time_avg(vgtk_comm_matrix_t *comm_matrix) {
   if (comm_matrix->nprocs == 0) {return;}
   int nprocs = comm_matrix->nprocs;
   ...
   // search for maximum / minimum average time
   //
   // first find the first valid entry
   comm_matrix->any_entry_valid = false;
   int first_valid = -1;
...
void comm_matrix_update_time_min(vgtk_comm_matrix_t *comm_matrix) {
   if (comm_matrix->nprocs == 0) {return;}
   int nprocs = comm_matrix->nprocs;
   double tmin = get_tmin_stacktimeline_draw();
   double tmax = get_tmax_stacktimeline_draw();
   ...
   // search for maximum / minimum time
   //
   // first find the first valid entry
   comm_matrix->any_entry_valid = false;
   int first_valid = -1;
