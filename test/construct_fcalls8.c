...
int main(int argc, char **argv) {
   ...
   vfd->header->function_samplecount = 3;
   ...
   vfd->stack_samples[0].kind = fnct_exit;
   vfd->stack_samples[0].stackID = 3;
   vfd->stack_samples[0].sampletime = 4000000ll;

   vfd->stack_samples[1].kind = fnct_entry;
   vfd->stack_samples[1].stackID = 5;
   vfd->stack_samples[1].sampletime = 7000000ll;

   vfd->stack_samples[2].kind = fnct_exit;
   vfd->stack_samples[2].stackID = 0;
   vfd->stack_samples[2].sampletime = 11000000ll;
   ...
   check_fcall(vfd->fcalls[0], 3, 0.0,  4.0);
   check_fcall(vfd->fcalls[1], 2, 0.0,  5.5);
   check_fcall(vfd->fcalls[2], 1, 0.0,  9.0);
   check_fcall(vfd->fcalls[3], 0, 0.0, 11.0);
   check_fcall(vfd->fcalls[4], 4, 5.5,  9.0);
